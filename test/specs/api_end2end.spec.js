...
describe('end-2-end',  () =>{
    ...
    it('should update user_1 and get verification message', async () => {
        const token = (await login_getUserTokenAndData(apiCredentials.email2, apiCredentials.pw2)).token;
        const email = (await login_getUserTokenAndData(apiCredentials.email2, apiCredentials.pw2)).email;
        const verification = await passwordResetRequest(token, email)
